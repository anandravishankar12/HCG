...
&and yields a derived image for each sigma value specified&\\
\hline
...
To define a Region of Interest (ROI) and to check the dimensional constrainsts of the data, a mask file is utilized. The mask image corresponding to Figure $\ref{img2}$ is shown in Figure $\ref{img3}$. Note the red mark demarcating the tumor is done by a radiologist as is standard procedure. The features are now extracted from the image set with the help of the mask file. The features extracted are descibed by the Imaging Biomarker Standardization Initiative (IBSI). The entire feature set has been shown in tables $\ref{tb3}$ and $\ref{tb4}$.
