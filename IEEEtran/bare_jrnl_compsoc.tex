...
\begin{abstract}
With the advent of modern medical scanning techniques such as PET-CT scans, the amount of information-rich medical data generated every day has risen exponentially. Medical centres are often overwhelmed with the influx of copius medical data and require a helping hand. The field of Machine Learning (ML) and Data Science provides assistance to those fields which contains an abundance of data and aims to detect inherent patterns from the datasets provided. Although ML tools can never replace the expertise offered by specialists, these tools when combined with sister fields like Radiomics, create a dynamic pipeline for future datasets. This pipeline accepts medical images as an input, exatracts Radiomics features from it, performs pre-processing tasks and applys ML tools for classification of the data into target classes. The final result so obtained consists of reduced graphical representations which provide insight into the mapping between the image feature space and taget class. Thw work described in this paper outlines a multi-stage process with emphasis on feature extraction and using them to reduce the target class space for oncological patients. 
\end{abstract}
